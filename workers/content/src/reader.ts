...
readerRouter.post('/:slug/verify-firebase', async (c) => {
  const slug = c.req.param('slug');
  const body = await c.req.json().catch(() => ({}));
  const idToken = body.idToken;
  const projectId = (c.env.FIREBASE_PROJECT_ID as string | undefined) || 'epaperspace';
  ...
  try {
    ...
    const token = await signReaderToken(c, readerId, slug, readerEmail);
